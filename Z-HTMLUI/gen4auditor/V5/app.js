...
(function () {
    ...
    document.body.insertAdjacentHTML("afterbegin", `
      <div class="app">
        <nav class="sidebar">
          ...
          ${nav}
          <div class="api-note" title="${esc(CFG.apiBaseUrl)}">Build ${esc(CFG.version || "")}${CFG.builds?.[0] ? " · " + esc(CFG.builds[0].title) : ""}<br>API: ${esc(new URL(CFG.apiBaseUrl).host.split(".")[0])}</div>
        </nav>
  ...
  window.Cocky = { appRollup, unlinkedHosts, removeAppLinks, CFG, SCHEMA, PAGES, api, layout, toast, errorBox, spinner, renderTable, filterRows, openRecordForm, confirmDelete, crudPage, esc, titleCase, fmtValue, parseDate, dateMs, ensureRefs, refText, isLiveException, hostException };
})();

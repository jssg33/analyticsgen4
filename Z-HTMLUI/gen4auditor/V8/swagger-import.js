...
(function () {
  ...
  const opKey = (method, path) => `${method || ""} ${path}`;
  ...
  function reconcile(parsed, host, all) {
  ...
  function dupeNote(dupes) {
    ...
    return `<b>${extra}</b> endpoint record(s) are duplicates of another record for the same operation (nothing new was added for them). ` +
      dupes.slice(0, 5).map(g => esc(`${methodOf(g[0]) || "?"} ${g[0].url || ""}: #${g.map(e => e.id).join(", #")}`)).join(" · ") +
  ...
  function open(host = null) {
    ...
    function renderPreview() {
      const rec = reconcile(parsed, targetHost, existing);
      parsed.endpoints.forEach(o => { o.pick = !o.exists; });
      ...
      $("#sw_summary").innerHTML = `
        ...
        <div class="small mt-1">${parsed.endpoints.length} operations · <b>${nNew} new</b> · ${parsed.endpoints.length - nNew} already registered${rec.adopt.length ? ` (${rec.adopt.length} will be linked to this host)` : ""} ·
      ...
      if (rec.dupes.length) $("#sw_removed").insertAdjacentHTML("beforeend", `<div class="alert alert-info small mb-0 mt-2">${dupeNote(rec.dupes)}</div>`);
    ...
    function drawRows() {
      ...
      $("#sw_rows").innerHTML = shown.map(o => {
        ...
        return `<tr class="${o.exists ? "text-muted" : ""}">
          ...
          <td class="small">${o.exists ? `Registered <span class="text-muted">#${esc(o.existing.id)}</span>` : '<span class="text-success">New</span>'}${o.deprecated ? ' <span class="badge text-bg-secondary">deprecated</span>' : ""}</td></tr>`;
    ...
    async function doImport() {
      ...
      out.innerHTML = Cocky.spinner("Checking which endpoints are already registered…");
      let linked = 0, skipped = 0;
      try {
        existing = await api.list("apiaudit");
        const fresh = reconcile(parsed, targetHost, existing);
        const before = picks.length;
        for (let k = picks.length - 1; k >= 0; k--) if (picks[k].exists) picks.splice(k, 1);
        const keys = new Set(fresh.missing.map(o => o.key));
        for (let k = picks.length - 1; k >= 0; k--) if (!keys.has(picks[k].key)) picks.splice(k, 1);
        skipped = before - picks.length;
        for (const e of fresh.adopt) {
          try { await api.update("apiaudit", e.id, { ...e, apiHostId: targetHost.id, modifiedDate: new Date().toISOString() }); linked++; }
          catch (err2) { failures.push(`Linking #${e.id}: ${err2.message}`); }
        }
      } catch (e) { out.innerHTML = `<div class="alert alert-danger">Couldn't re-check existing endpoints, so nothing was imported: ${esc(e.message)}</div>`; return; }
      ...
      if (created || deactivated || linked) changed = true;
      bar.style.width = "100%";
      out.innerHTML = `
        <div class="alert alert-${failures.length ? "warning" : "success"}">
          <b>${created}</b> new endpoint(s) imported${deactivated ? `, <b>${deactivated}</b> marked inactive` : ""} for host <b>#${targetHost.id} ${esc(targetHost.apiHostName || "")}</b>.
          ${skipped ? `<div class="small mt-1">${skipped} selected operation(s) were already registered by the time of import and were skipped.</div>` : ""}
          ${linked ? `<div class="small mt-1">${linked} existing endpoint(s) linked to this host instead of being added again.</div>` : ""}
  ...
  // Before Audit host writes anything: show how many endpoint records will be inserted (and linked), and ask.
  // Resolves "add" (insert and audit), "skip" (audit only what's registered, write nothing) or null (cancel).
  function confirmChanges(host, parsed, missing, adopt) {
    let el = document.getElementById("insModal");
    if (!el) {
      document.body.insertAdjacentHTML("beforeend", `
      <div class="modal fade" id="insModal" tabindex="-1" data-bs-backdrop="static"><div class="modal-dialog modal-lg modal-dialog-scrollable"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Add new endpoints?</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
          <p class="mb-2" id="ins_sum"></p>
          <div class="row g-2 text-center mb-3" id="ins_counts"></div>
          <div id="ins_list"></div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary me-auto" data-bs-dismiss="modal">Cancel</button>
          <button type="button" class="btn btn-outline-primary" data-choice="skip">Audit without adding</button>
          <button type="button" class="btn btn-primary" data-choice="add" id="ins_add"></button>
        </div>
      </div></div></div>`);
      el = document.getElementById("insModal");
    }
    ...
    el.querySelector("#ins_sum").innerHTML = `Swagger for <b>${esc(host.apiHostName || host.apiHostUrl)}</b> lists <b>${total}</b> operations. ` +
      `<b>${have}</b> are already registered and won't be added again.`;
    el.querySelector("#ins_counts").innerHTML = tile(missing.length, "new records to insert", missing.length ? "text-success" : "") +
      tile(adopt.length, "existing records to link to this host") + tile(have, "already registered");
    const rows = missing.map(o => `<tr><td>${methodBadge(o.method)}</td><td class="font-monospace small">${esc(o.path)}</td><td class="small">${esc(o.tag)}</td></tr>`).join("");
    el.querySelector("#ins_list").innerHTML = (missing.length ? `<h6 class="small text-uppercase text-muted">Will be inserted</h6>
      <div class="table-responsive" style="max-height:260px"><table class="table table-sm mb-2"><tbody>${rows}</tbody></table></div>` : "") +
      (adopt.length ? `<div class="small text-muted">${adopt.length} registered endpoint(s) match these operations but aren't linked to this host; they'll get <code>apiHostId ${esc(host.id)}</code>. No copies are made.</div>` : "");
    el.querySelector("#ins_add").textContent = missing.length ? `Insert ${missing.length} record${missing.length === 1 ? "" : "s"} & audit` : `Link ${adopt.length} & audit`;
    return credIdle.then(() => new Promise(resolve => {
      let result = null;
      const bs = bootstrap.Modal.getOrCreateInstance(el);
      el.querySelectorAll("[data-choice]").forEach(b => b.onclick = () => { result = b.dataset.choice; bs.hide(); });
      el.addEventListener("hidden.bs.modal", () => resolve(result), { once: true });
      credIdle = new Promise(r => el.addEventListener("hidden.bs.modal", r, { once: true }));
      bs.show();
    }));
  }
  ...
  window.SwaggerImport = { open, parseSpec, findSpec, walkHost, promptCredentials, pushHostDetails, hostDefaults, reconcile, dupeNote };
})();
